...
class RenameGroup:
    ...
    groupList = MainClass.findInRangeBelow(Resource.imgSubContactTab, 580).highlight(1)

    while not groupList.exists(Resource.imgGroupName):
        scrollArea = MainClass.findInRangeLeft(Resource.imgStickerScroll, 285)
        wheel(scrollArea, WHEEL_DOWN, 10)

    rightClick(Resource.imgGroupName)
    wait(1)
    click(Resource.imgDoiTenNhom)
...
class DeleteGroup:
    ...
    listMember = MainClass.findInRangeBelow(Resource.imgSearchBar, 655).highlight(1)
